...
async def test_adapter_construction_does_not_leak_scoped_secret_into_ambient_env(
    ...
    PR #9 round 3's BLOCK-C): the constructor's own `lease_seconds`
    resolution (`_configured_mcp_tool_timeout` -> `hermes_cli.config.
    load_config` -> `hermes_cli.mcp_config._resolve_mcp_server_config`) ran
    OUTSIDE any profile secret scope. `_resolve_mcp_server_config` itself
    falls back to `load_hermes_dotenv()` -- loading `.env` INTO `os.environ`
    -- exactly when `current_secret_scope() is None`, which is true again by
    ...
    `MupotAdapter` -- registration-time scoping does not cover
    ...
    scoped secret -- a multiplex cross-tenant leak into shared process state
    that any other, unscoped code reading `os.environ` directly (rather than
    through `read_profile_secret`) would then see. Fixed by resolving the
    lease-sizing formula's MCP tool timeout inside `self._profile_scope()`,
    which activates `secret_owner` again for the duration of just this read
    ...
    try:
        assert os.environ.get("MUPOT_AGENT_TOKEN") == GLOBAL_AGENT_SECRET, (
            "P1-5: constructing the adapter must never overwrite the "
